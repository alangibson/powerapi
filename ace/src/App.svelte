<script lang="ts">
  ...
  // @ts-ignore because this is provided by all.js
  ...
  function detectDocType(schema, defaultType) {
    try {
      ...
      if (schema.openapi) return ["openapi", schema.openapi];
      else if (schema.swagger) return ["openapi", schema.swagger];
      else if (schema.asyncapi) return ["asyncapi", schema.asyncapi];
      else return [defaultType, null];
    } catch (e) {
  ...
  function loadParser(text, docFormat) {
    try {
      if (docFormat == "json") return [JSON.parse(text), null];
      else if (docFormat == "yaml") return [yaml.load(text), null];
      else return [null, `Document format ${docFormat} not supported yet`];
    } catch (e) {
      return [null, e];
    }
  }
  ...
  const autoLanguage = EditorState.transactionExtender.of((tr) => {
    ...
    if (docFormat == "json")
      ...
      return {
        effects: languageConf.reconfigure(StreamLanguage.define(yamlMode)),
      };
    else return null;
  ...
  function confluenceSave() {
    console.debug(
      `[PowerPlantUML] Saving macro. Body length is ${text.length}`
    );
  ...
  function confluenceCancel() {
    console.debug(`Cancel and go back to Confluence`);
    AP.confluence.closeMacroEditor();
  }
  ...
  <div id="editor">
    ...
    <div id="preview">
      {#if !valid && (parseError || validationError)}
        <div
          id="errors"
          style="background-color: rgba(255,0,0,0.2); padding: 8px"
        >
          <h2>Errors</h2>
